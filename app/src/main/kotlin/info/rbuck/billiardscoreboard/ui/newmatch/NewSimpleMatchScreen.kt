...
fun NewSimpleMatchScreen(
    ...
    val defaultRaceTo = remember(gameType) {
        when (gameType) {
            GameType.EIGHT_BALL -> app.settingsRepository.defaultRace8Ball.value
    ...
    var breakMode by rememberSaveable { mutableStateOf(BreakMode.ALTERNATE) }
    var firstBreakPlayer by rememberSaveable { mutableStateOf(0) }
    ...
    LaunchedEffect(rematchOfMatchId) {
        ...
        val previous = (app.matchRepository.loadPayload(id) as? MatchStatePayload.Simple)?.state ?: return@LaunchedEffect
        player1Id = previous.playerIds.getOrNull(0)
        player2Id = previous.playerIds.getOrNull(1)
        raceTo = previous.settings.raceTo
        breakMode = previous.settings.breakMode
        firstBreakPlayer = previous.settings.firstBreakPlayer
    ...
    val maxHandicap = SimpleMatchEngine.maxHandicapFor(raceTo)
    ...
    val startMatch = {
        ...
        viewModel.createSimpleMatch(
            gameType = gameType,
            player1 = player1!!,
            player2 = player2!!,
            raceTo = raceTo,
            breakMode = breakMode,
            firstBreakPlayer = firstBreakPlayer,
            handicap = handicap,
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    MatchSetupHeaderTitle(gameType = gameType, onGameTypeSelected = onGameTypeSelected)
                },
                navigationIcon = {
